...
//     int prevPos = -1;  // Track previous position to detect infinite loops

//     while (true) {
//         // ✅ Acceptance condition
//         if (pos < tokens.size() &&
//             tokens[pos].type == TokenType::END &&
//             stack.size() == 1 &&
...
//         // ✅ Prevent stuck loop (same position & no progress)
//         if (pos == prevPos && !tryReduce()) {
//             syntaxError("parser got stuck — no valid reduction or shift");
//             return false;
//         }
//         prevPos = pos;
...
//                 for (auto &item : stack) {
//                     if (!item.node->errorMsg.empty()) {
//                         storedError = item.node->errorMsg;
//                         hasError = true;
//                         break;
//                     }
//                 }
...
//                 if (hasError)
//                     syntaxError(storedError);
//                 else
//                     syntaxError("incomplete or invalid statement");
    ...
    while (true) {
        ...
        if (!reduced) {
            ...
            if (tokens[pos].type == TokenType::END) {
                ...
                // fallback message based on stack top
                // if (stack.empty()) {
                //     syntaxError("empty input", pos);
                // } else if (stack.back().symbol == "<expr>" ||
                //            stack.back().symbol == "<term>" ||
                //            stack.back().symbol == "<factor>" ||
                //            stack.back().symbol == "id" ||
                //            stack.back().symbol == "int" ||
                //            stack.back().symbol == ")") {
                //     syntaxError("missing ';'", pos);
                // } else {
                //     syntaxError("incomplete statement", pos);
                // }
                // return false;
            ...
            if (current == "+" || current == "-" || current == "*" || current == "/") {
                ...
                if (!topIsOperand) {
                    syntaxError("expected operand before '" + current + "'", pos);
                    return false;
                }
                if (!nextIsOperand) {
                    syntaxError("expected operand before '" + next + "'", pos);
                    return false;
                }
            }

            // If next is ')' but there's no matching '(' on stack (or top is operator) -> error
            if (next == ")") {
                // quick heuristic: top must be an expression (or ')' or 'int' etc.)
                bool topIsExpr = (top == "<expr>" || top == "<term>" ||
                                  top == "<factor>" || top == "id" ||
                                  top == "int" || top == ")");
                if (!topIsExpr) {
                    syntaxError("unexpected ')'", pos);
                    return false;
                }
            }

            // If next is '(' but previous token is an operand without operator -> error
            // if (next == "(") {
            //     bool prevIsOperand = (top == "<expr>" || top == "<term>" ||
            //                           top == "<factor>" || top == "id" ||
            //                           top == "int" || top == ")");
            //     if (prevIsOperand) {
            //         syntaxError("missing operator before '('", pos);
            //         return false;
            //     }
            // }
            ...
            if (look.type == TokenType::END) {
                if (top == "+" || top == "-" || top == "*" || top == "/") {
                    // The last token shifted was an operator, but there's no right operand
                    syntaxError("expected operand after '" + top + "'", pos);
                    return false;
                }
                // ... rest of END handling (missing ';', etc.) ...
            }
...
//             // ⚠️ --- Syntax error detection inside expressions ---
//             if (lhs == "<expr>") {
//                 if (rhs == vector<string>{"<expr>", "+", "<term>"} ||
//                     rhs == vector<string>{"<expr>", "-", "<term>"}) {

//                     Node* left = parent->children[0];
//                     Node* right = parent->children[2];

//                     if (left->label.empty() || right->label.empty()) {
//                         parent->errorMsg = "missing operand near '+' or '-'";
//                     }
//                 }
//             } else if (lhs == "<term>") {
//                 if (rhs == vector<string>{"<term>", "*", "<factor>"} ||
//                     rhs == vector<string>{"<term>", "/", "<factor>"}) {
...
//                     }
//                 }
//             } else if (lhs == "<factor>" && rhs == vector<string>{"(", "<expr>", ")"}) {
//                 Node* exprNode = parent->children[1];
//                 if (!exprNode || exprNode->label != "<expr>") {
//                     parent->errorMsg = "missing or invalid expression inside parentheses";
//                 }
//             }

//             // --- Pop stack and push reduced node ---
//             for (int i = 0; i < rhs.size(); i++)
//                 stack.pop_back();
//             stack.push_back({lhs, parent});
